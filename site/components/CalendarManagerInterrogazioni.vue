...
<div>
    <b-modal
      v-model="show"
      title="Schedula turno interrogazione"
    ...
    >
      <b-container fluid>
        ...
        <b-row class="mb-1">
          ...
          <br>
          <b-row class="mb-1 text-center">
          <label >Numero Interrogati</label>
          <b-col>
            
            <b-form-input
              v-model="numero_interrogati"
              style="width:100%;"
              ></b-form-input> 
          </b-col>
        </b-row>
      ...
      <template v-slot:modal-footer>
        <div class="w-100">
          ...
          <p class="float-left">Confermando verrà schedulato un turno di interrogazioni</p>
          <b-button
            variant="primary"
            size="sm"
            class="float-right"
    ...
    export default {
        ...
        data() {
            return {
                ...
                numero_interrogati:0,
                oraselezionata: 8,
                giornoselezionato: 0,
                materiaselezionata: null,
                textmateriaselezionata: '',
                listaMaterie:[
        ...
        mounted(){              
          ...
          methods: {
                ...
                handleEventClick(arg) {
                  ...
                  // for (let key in this.eventSelected){ 
                  //   if (this.listaMaterie[key].value === this.eventSelected){
                  //   }
                  // }
                  var buff_options = this.listaMaterie.find(x => x.text === arg.event.title)
                  
                  this.materiaselezionata = buff_options.value
                  this.textmateriaselezionata = arg.event.title
                  this.dataselezionata = arg.event.start
                ...
                async submitNuovoOrario(){
                  ...
                  formData.append('numero_interrogati',this.numero_interrogati)
                  formData.append('data',this.dataselezionata.toISOString())
                  try {
                    let response = await this.$axios.$post("api/classe/turnointerrogazione/", formData, config);
