  ...
  <main class="container my-5">
    <div class="row">
      <div class="col-12 text-center my-3">
        <h2 class="mb-3 display-4">Aggiungi una classe</h2>
      </div>
      ...
      <div class="col-md-4">
        <form @submit.prevent="submitRecipe">
          ...
          <div class="form-group mb-3">
            <label for>Note Iniziali</label>
            <textarea v-model="classe.note" class="form-control" rows="8"></textarea>
          </div>
          <button type="submit" class="btn btn-primary">Crea nuova classe</button>
        </form>
      </div>
    </div>
  </main>
...
export default {
  ...
  methods: {
    ...
    async submitRecipe() {
      ...
      try {
        let response = await this.$axios.$post("api/classe/", formData, config);
